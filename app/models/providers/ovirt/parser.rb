class Providers::Ovirt::Parser
  ...
  def self.storage_inv_to_hashes(inv)
    ...
    inv.each do |storage_inv|
      ...
      ems_ref = Providers::Ovirt::Manager.make_ems_ref(storage_inv[:href])
  ...
  def self.host_inv_to_hashes(inv, ems_inv, cluster_uids, _storage_uids)
    ...
    inv.each do |host_inv|
      ...
      ems_ref = Providers::Ovirt::Manager.make_ems_ref(host_inv[:href])
      new_result = {
        :type             => 'Providers::Ovirt::Host',
        :ems_ref          => ems_ref,
        :ems_ref_obj      => ems_ref,
  ...
  def self.host_inv_to_ip(inv, hostname = nil)
    ...
    Array.wrap(inv[:host_nics]).each do |nic|
      ip_data = nic[:ip]
  ...
  def self.host_inv_to_hardware_hash(inv)
    ...
    unless hdw.blank?
      ...
      memory_total = Array.wrap(inv[:statistics]).detect { |stat| stat[:name] == 'memory.total' }
  ...
  def self.vm_inv_to_hashes(inv, _storage_inv, storage_uids, cluster_uids, host_uids, lan_uids)
    ...
    inv.each do |vm_inv|
      ...
      storages = []
      Array.wrap(vm_inv[:disks]).each do |disk|
      ...
      ems_ref = Providers::Ovirt::Manager.make_ems_ref(vm_inv[:href])
  ...
  def self.partial_host_hash(partial_host_inv)
    ems_ref = Providers::Ovirt::Manager.make_ems_ref(partial_host_inv[:href])
    {
      :ems_ref     => ems_ref,
  ...
  def self.create_vm_hash(template, ems_ref, vm_id, name)
    {
      :type        => template ? "Providers::Ovirt::Template" : "Providers::Ovirt::Vm",
      :ems_ref     => ems_ref,
      :ems_ref_obj => ems_ref,
      :uid_ems     => vm_id,
      :vendor      => "redhat",
  ...
  def self.vm_inv_to_network_hashes(inv, guest_device_uids)
    ...
    Array.wrap(inv_net).each do |data|
      new_result = {}
  ...
  def self.vm_inv_to_disk_hashes(inv, storage_uids)
    ...
    inv = Array.wrap(inv).sort_by do |disk|
  ...
  def self.vm_inv_to_snapshot_hashes(inv)
    result = []
    inv = Array.wrap(inv[:snapshots]).reverse
    return result if inv.nil?
  ...
  def self.cluster_inv_to_hashes(inv)
    ...
    inv.each do |data|
      ...
      ems_ref = Providers::Ovirt::Manager.make_ems_ref(data[:href])
  ...
  def self.datacenter_inv_to_hashes(inv, cluster_uids, vm_uids, storage_uids, host_uids)
    ...
    inv.each do |data|
      ...
      storage_ids = Array.wrap(data[:storagedomains]).collect { |s| s[:id] }
